...
if uploaded_file is not None:
    try:
        ...
        # ✨ THE FIX YOU JUST ASKED FOR! Perfectly formatted with closing brackets ✨
            ...
            with st.spinner("Importing data... This might take a minute."):
                ...
                for index, row in df.iterrows():
                    try:
                        ...
                        # ✨ FIX 3: THE ULTIMATE CATCH-ALL! (Catches the sneaky Excel double-space: "Purchase  Mode")
                        payment_val = str(row.get('Purchase  Mode', row.get('Purchase Mode', row.get('Purchase Type', row.get('Payment Mode', row.get('Payment', row.get('Payment Type', '-'))))))).strip()
